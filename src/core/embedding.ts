...
async function getRemoteEmbedding(
    ...
    try {
        ...
        if (!response.ok) {
            tgAgenticRagLogger.logError("API Response:", await response.text()); // Debug log
            throw new Error(
        ...
        tgAgenticRagLogger.logError("Full error details:", e);
        throw e;
    }
}
...
export async function embed(runtime: IAgentRuntime, input: string) {
    tgAgenticRagLogger.logDebug("Embedding request:", {
    ...
    if (!input || typeof input !== "string" || input.trim().length === 0) {
        tgAgenticRagLogger.logWarn("Invalid embedding input:", {
            input,
            type: typeof input,
            length: input?.length,
        });
    ...
    async function getLocalEmbedding(input: string): Promise<number[]> {
        tgAgenticRagLogger.logDebug("DEBUG - Inside getLocalEmbedding function");
        ...
        try {
            const moduleImports = await Promise.all([
                ...
                (async () => {
                    try {
                        ...
                        tgAgenticRagLogger.logError("Failed to load fastembed.");
                        throw new Error(
            ...
            tgAgenticRagLogger.logDebug("Initializing BGE embedding model...");
            ...
            tgAgenticRagLogger.logDebug("Generating embedding for input:", {
                inputLength: input.length,
            ...
            tgAgenticRagLogger.logDebug("Raw embedding from BGE:", {
            ...
            tgAgenticRagLogger.logDebug("Processed embedding:", {
    ...
    async function retrieveCachedEmbedding(
        ...
        if (!input) {
            tgAgenticRagLogger.logInfo("No input to retrieve cached embedding for");
            return null;
        }
