...
} from "./templatesAndParsings";


import { tgAgenticRagLogger } from "../logger";
import { IAgentRuntime, ModelClass, ModelProviderName, ServiceType,
    ITextGenerationService, Content, IImageDescriptionService,
    SearchResponse,
    ActionResponse, 
...
export async function generateText({
    ...
    if (!context) {
        console.log("generateText context is empty");
        return "";
    }

    tgAgenticRagLogger.logInfo("Generating text...");

    tgAgenticRagLogger.logInfo("Generating text with options:", {
    ...
    tgAgenticRagLogger.logInfo("Selected model:", model);
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.AKASH_CHAT_API: {
                tgAgenticRagLogger.logDebug("Initializing OpenAI model.");
                ...
                tgAgenticRagLogger.logDebug("Received response from OpenAI model.");
                break;
            }
            ...
            case ModelProviderName.GOOGLE: {
                ...
                tgAgenticRagLogger.logDebug("Received response from Google model.");
                break;
            }

            case ModelProviderName.ANTHROPIC: {
                tgAgenticRagLogger.logDebug("Initializing Anthropic model.");
                ...
                tgAgenticRagLogger.logDebug("Received response from Anthropic model.");
                break;
            }

            case ModelProviderName.CLAUDE_VERTEX: {
                tgAgenticRagLogger.logDebug("Initializing Claude Vertex model.");
            ...
            case ModelProviderName.GROK: {
                tgAgenticRagLogger.logDebug("Initializing Grok model.");
                ...
                tgAgenticRagLogger.logDebug("Received response from Grok model.");
                break;
            }

            case ModelProviderName.GROQ: {
                const groq = createGroq({ apiKey: apiKey || undefined, fetch: runtime.fetch || undefined });
            ...
            case ModelProviderName.LLAMALOCAL: {
                ...
                tgAgenticRagLogger.logDebug("Received response from local Llama model.");
                break;
            }

            case ModelProviderName.REDPILL: {
                tgAgenticRagLogger.logDebug("Initializing RedPill model.");
                ...
                tgAgenticRagLogger.logDebug("Received response from redpill model.");
                break;
            }

            case ModelProviderName.OPENROUTER: {
                tgAgenticRagLogger.logDebug("Initializing OpenRouter model.");
                const serverUrl = models[provider].endpoint;
                ...
                tgAgenticRagLogger.logDebug("Received response from OpenRouter model.");
                break;
            }

            case ModelProviderName.OLLAMA:
                {
                    tgAgenticRagLogger.logDebug("Initializing Ollama model.");
                    ...
                    tgAgenticRagLogger.logDebug("****** MODEL\n", model);
                ...
                tgAgenticRagLogger.logDebug("Received response from Ollama model.");
                break;

            case ModelProviderName.HEURIST: {
                tgAgenticRagLogger.logDebug("Initializing Heurist model.");
                ...
                tgAgenticRagLogger.logDebug("Received response from Heurist model.");
                break;
            }
            case ModelProviderName.GAIANET: {
                tgAgenticRagLogger.logDebug("Initializing GAIANET model.");
                ...
                tgAgenticRagLogger.logDebug("Using GAIANET model with baseURL:", baseURL);
                ...
                response = openaiResponse;
                tgAgenticRagLogger.logDebug("Received response from GAIANET model.");
                break;
            }

            case ModelProviderName.GALADRIEL: {
                tgAgenticRagLogger.logDebug("Initializing Galadriel model.");
                ...
                tgAgenticRagLogger.logDebug("Received response from Galadriel model.");
                break;
            }

            case ModelProviderName.VENICE: {
                tgAgenticRagLogger.logDebug("Initializing Venice model.");
                ...
                tgAgenticRagLogger.logDebug("Received response from Venice model.");
                break;
            }

            default: {
                const errorMessage = `Unsupported provider: ${provider}`;
                tgAgenticRagLogger.logError(errorMessage);
        ...
        tgAgenticRagLogger.logError("Error in generateText:", error);
        throw error;
    }
}
...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            tgAgenticRagLogger.logInfo("Received response from generateText:", response);
            const parsedResponse = parseShouldRespondFromText(response.trim());
            if (parsedResponse) {
                tgAgenticRagLogger.logDebug("Parsed response:", parsedResponse);
                return parsedResponse;
            } else {
                tgAgenticRagLogger.logInfo("generateShouldRespond no response");
            }
        } catch (error) {
            tgAgenticRagLogger.logError("Error in generateShouldRespond:", error);
            if (
        ...
        tgAgenticRagLogger.logInfo(`Retrying in ${retryDelay}ms...`);
...
export async function generateTrueOrFalse({
    ...
    while (true) {
        try {
            ...
            tgAgenticRagLogger.logError("Error in generateTrueOrFalse:", error);
        }
...
export async function generateTextArray({
    ...
    if (!context) {
        tgAgenticRagLogger.logError("generateTextArray context is empty");
        return [];
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            tgAgenticRagLogger.logError("Error in generateTextArray:", error);
        }
...
export async function generateObjectDeprecated({
    ...
    if (!context) {
        tgAgenticRagLogger.logError("generateObjectDeprecated context is empty");
        return null;
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            tgAgenticRagLogger.logError("Error in generateObject:", error);
        }
...
export async function generateObjectArray({
    ...
    if (!context) {
        tgAgenticRagLogger.logError("generateObjectArray context is empty");
        return [];
    }
    let retryDelay = 1000;
...
export async function generateMessageResponse({
    ...
    while (true) {
        try {
            tgAgenticRagLogger.logInfo("Generating message response..");
            ...
            if (!parsedContent) {
                tgAgenticRagLogger.logInfo("parsedContent is null, retrying");
                continue;
            }

            return parsedContent;
        } catch (error) {
            tgAgenticRagLogger.logError("ERROR:", error);
            // wait for 2 seconds
            retryLength *= 2;
            await new Promise((resolve) => setTimeout(resolve, retryLength));
            tgAgenticRagLogger.logInfo("Retrying...");
        }
    }
}
...
export const generateImage = async (
    ...
    tgAgenticRagLogger.logInfo("Generating image with options:", {
        ...
        if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
            ...
            const base64s = await Promise.all(
                togetherResponse.data.map(async (image) => {
                    if (!image.url) {
                        tgAgenticRagLogger.logError("Missing URL in image data:", image);
            ...
            tgAgenticRagLogger.logDebug(`Generated ${base64s.length} images`);
            ...
            const result = await fal.subscribe(model || 'fal-ai/fast-sd', {
                input,
                logs: true,
                onQueueUpdate: (update) => {
                    if (update.status === "IN_PROGRESS") {
                        tgAgenticRagLogger.logInfo(update.logs.map((log) => log.message));
                    }
                },
            });
            ...
            try {
                ...
                const base64Images = await Promise.all(
                    result.images.map(async (image) => {
                        console.info("imageUrl console log", image.url);
                        let imageUrl;
...
export const generateWebSearch = async (
    ...
    try {
        ...
        if (!response.ok) {
            throw new tgAgenticRagLogger.logError(
    ...
    while (true) {
        try {
            ...
            if (actions) {
                ...
                tgAgenticRagLogger.logDebug("generateTweetActions no valid response");
            }
        } catch (error) {
            tgAgenticRagLogger.logError("Error in generateTweetActions:", error);
            if (
                error instanceof TypeError &&
                error.message.includes("queueTextCompletion")
            ) {
