import { IAgentRuntime, Character, UUID,
    IDatabaseAdapter,
    ...
    KnowledgeItem,
    State,
    Memory, HandlerCallback,
    ModelClass, Actor, Goal,
    Media,
 } from "./types";
 import { stringToUuid } from "../utils/uuid";
 import { v4 as uuidv4 } from "uuid";
 import { tgAgenticRagLogger } from "../logger";
 import { defaultCharacter } from "../agents/characters/defaultCharacter";
 import { MemoryManager } from "./memory";
 import knowledge from "./knowledge";
 import { settings } from "./settings";
 import { composeContext } from "./composeContext"; 
 import { generateText } from "./generations";
 import { parseJsonArrayFromText, evaluationTemplate, addHeader,  } from "./templatesAndParsings";
 import { 
...
export class AgentRuntime implements IAgentRuntime {
    ...
    getService<T extends Service>(service: ServiceType): T | null {
        ...
        if (!serviceInstance) {
            tgAgenticRagLogger.logError(`Service ${service} not found`);
            return null;
        }
    ...
    async registerService(service: Service): Promise<void> {
        ...
        tgAgenticRagLogger.logInfo("Registering service:", serviceType);
        ...
        tgAgenticRagLogger.logSuccess(`Service ${serviceType} registered successfully`);
    }
    ...
    constructor(opts: {
        ...
        tgAgenticRagLogger.logInfo("Initializing AgentRuntime with options:", {
        ...
        tgAgenticRagLogger.logSuccess("Agent ID", this.agentId);
        ...
        tgAgenticRagLogger.logInfo("Setting model provider...");
        ...
        tgAgenticRagLogger.logInfo("Model Provider Selection:", {
        ...
        tgAgenticRagLogger.logInfo("Selected model provider:", this.modelProvider);
        ...
        if (!Object.values(ModelProviderName).includes(this.modelProvider)) {
            tgAgenticRagLogger.logError("Invalid model provider:", this.modelProvider);
        ...
        if (!this.serverUrl) {
            tgAgenticRagLogger.logWarn("No serverUrl provided, defaulting to localhost");
        }
    ...
    async stop() {
      tgAgenticRagLogger.logDebug('runtime::stop - character', this.character)
      ...
      for(const cStr in this.clients) {
        ...
        tgAgenticRagLogger.logInfo('runtime::stop - requesting', cStr, 'client stop for', this.character.name)
        c.stop()
      }
    ...
    registerAction(action: Action) {
        tgAgenticRagLogger.logSuccess(`Registering action: ${action.name}`);
    ...
    async processActions(
        ...
        for (const response of responses) {
            if (!response.content?.action) {
                tgAgenticRagLogger.logWarn("No action found in the response content.");
                continue;
            }
            ...
            tgAgenticRagLogger.logSuccess(`Normalized action: ${normalizedAction}`);
            ...
            if (!action) {
                tgAgenticRagLogger.logInfo("Attempting to find action in similes.");
            ...
            if (!action.handler) {
                tgAgenticRagLogger.logError(`Action ${action.name} has no handler.`);
                continue;
            }
            ...
            try {
                ...
                tgAgenticRagLogger.logError(error);
            }
        }
    }
    ...
    async evaluate(
        ...
        // log how many evaluators there are
        tgAgenticRagLogger.logInfo('Evaluating', this.evaluators.length, 'evaluators');

        const evaluatorPromises = this.evaluators.map(
            async (evaluator: Evaluator) => {
                tgAgenticRagLogger.logInfo("Evaluating", evaluator.name);
        ...
        if (evaluatorsData.length === 0) {
            tgAgenticRagLogger.logInfo('No evaluators found, returning empty array');
            return [];
        }
        ...
        const context = composeContext({
            state: {
                ...state,
                evaluators: formatEvaluators(evaluatorsData.filter((e): e is Evaluator => e !== null)),
            } as State,
            template:
    ...
    async ensureUserExists(
        ...
        if (!account) {
            ...
            tgAgenticRagLogger.logSuccess(`User ${userName} created successfully.`);
        }
    }
    ...
    async ensureRoomExists(roomId: UUID) {
        ...
        if (!room) {
            ...
            tgAgenticRagLogger.logInfo(`Room ${roomId} created successfully.`);
        }
    }
        ...
        if (recentMessagesData && Array.isArray(recentMessagesData)) {
            ...
            if (lastMessageWithAttachment) {
                ...
                const oneHourBeforeLastMessage =
                    (lastMessageTime ?? Date.now()) - 60 * 60 * 1000; // 1 hour before last message
...
Text: ${attachment.text}
  `
        ...
        const getRecentInteractions = async (
            ...
            tgAgenticRagLogger.logInfo('Getting memories by room ids', {
            ...
            tgAgenticRagLogger.logInfo('Recent interactions data', {
                recentInteractionsData: recentInteractionsData.map((memory) => ({
                    id: memory.id,
        ...
        const actionState = {
            ...
            evaluators:
                evaluatorsData.length > 0
                    ? formatEvaluators(evaluatorsData.filter((e): e is Evaluator => e !== null))
                    : "",
            evaluatorNames:
                evaluatorsData.length > 0
                    ? formatEvaluatorNames(evaluatorsData.filter((e): e is Evaluator => e !== null))
                    : "",
            evaluatorExamples:
                evaluatorsData.length > 0
                    ? formatEvaluatorExamples(evaluatorsData.filter((e): e is Evaluator => e !== null))
                    : "",
            providers: addHeader(
                `# Additional Information About ${this.character.name} and The World`,
                Array.isArray(providers) ? providers.filter((p): p is Provider => p !== null).join('\n') : providers
            ),
        };
