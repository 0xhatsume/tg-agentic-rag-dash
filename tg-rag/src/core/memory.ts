...
export class MemoryManager implements IMemoryManager {
    ...
    async addEmbeddingToMemory(memory: Memory): Promise<Memory> {
        ...
        try {
            ...
            tgAgenticRagLogger.logError("Failed to generate embedding:", error);
    ...
    async createMemory(memory: Memory, unique = false): Promise<void> {
        ...
        if (existingMessage) {
            tgAgenticRagLogger.logDebug("Memory already exists, skipping");
            return;
        }

        tgAgenticRagLogger.logDebug("Creating Memory", memory.id, memory.content.text);
